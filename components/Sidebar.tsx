...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen, boards, currentBoardId, onSelectBoard, onCreateBoard, onUpdateBoard, onDeleteBoard, isHidden = false, theme, onToggleTheme }) => {
  ...
  const handleUpdate = (id: string) => {
    if (editBoardName.trim()) {
        onUpdateBoard(id, editBoardName);
        setEditingBoardId(null);
    }
  };

  const handleDelete = (id: string) => {
    if (confirm("Are you sure you want to delete this board and its tasks?")) {
        onDeleteBoard(id);
        setEditingBoardId(null);
    }
  };
          ...
          <nav className="space-y-1.5 flex-1 overflow-y-auto no-scrollbar">
            ...
            {boards.map((board) => {
                if (editingBoardId === board.id) {
                    return (
                        <div key={board.id} className="flex items-center gap-1.5 px-1 py-1 animate-in fade-in zoom-in-95 duration-200">
                            <input 
                                autoFocus
                                className="flex-1 min-w-0 rounded-lg px-2.5 py-2 text-sm bg-white/30 dark:bg-white/5 border border-white/40 dark:border-white/10 text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:bg-white/50 dark:focus:bg-white/10 focus:border-white/60 dark:focus:border-white/20 transition-all"
                                value={editBoardName}
                                onChange={(e) => setEditBoardName(e.target.value)}
                                onKeyDown={(e) => { if(e.key === 'Enter') handleUpdate(board.id); }}
                            />
                            <button onClick={() => handleUpdate(board.id)} className="p-2 rounded-xl bg-white/40 dark:bg-slate-900/40 border border-white/40 dark:border-white/10 text-slate-800 dark:text-white hover:bg-white/60 dark:hover:bg-slate-900/60 hover:scale-105 transition-all shadow-sm">
                                <Check size={16} strokeWidth={2.5} />
                            </button>
                            <button onClick={() => handleDelete(board.id)} className="p-2 rounded-lg bg-red-500/20 text-red-600 dark:text-red-400 hover:bg-red-500/30 transition-colors">
                                <Trash2 size={16} />
                            </button>
                            <button onClick={() => setEditingBoardId(null)} className="p-2 rounded-lg text-slate-500 dark:text-white/40 hover:bg-white/20 transition-colors">
                                <X size={16} />
                            </button>
                        </div>
                    );
                ...
                return (
                    <div key={board.id} className="group relative flex items-center">
                        <button onClick={() => { onSelectBoard(board.id); setIsOpen(false); }} className={`${GLASS_MENU_ITEM} ${currentBoardId === board.id ? GLASS_MENU_ITEM_ACTIVE : GLASS_MENU_ITEM_INACTIVE} pr-8`}>
                            <List size={18} /><span className="font-medium text-sm truncate">{board.name}</span>
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); startEditing(board); }} className={`absolute right-2 p-1.5 rounded-md text-slate-400 dark:text-white/30 transition-all hover:text-slate-900 dark:hover:text-white hover:bg-white/20 dark:hover:bg-white/10 ${currentBoardId === board.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                            <Pencil size={14} />
                        </button>
                    </div>
                );
            ...
            {isCreating ? (
                <form onSubmit={handleSubmit} className="px-1 py-1 flex items-center gap-1.5 animate-in fade-in zoom-in-95 duration-200">
                    <input autoFocus type="text" placeholder="Board Name" value={newBoardName} onChange={(e) => setNewBoardName(e.target.value)} onBlur={() => !newBoardName && setIsCreating(false)} className="flex-1 min-w-0 rounded-lg px-2.5 py-2 text-sm focus:outline-none transition-all bg-white/30 dark:bg-white/5 border border-white/40 dark:border-white/10 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-white/20 focus:border-slate-400 dark:focus:border-white/30" />
                    <button type="submit" className="p-2 rounded-xl bg-white/40 dark:bg-slate-900/40 border border-white/40 dark:border-white/10 text-slate-800 dark:text-white hover:bg-white/60 dark:hover:bg-slate-900/60 hover:scale-105 transition-all shadow-sm"><Check size={18} strokeWidth={2.5} /></button>
                    <button type="button" onClick={() => setIsCreating(false)} className="p-2 rounded-lg text-slate-500 dark:text-white/40 hover:bg-white/20 transition-colors"><X size={18}/></button>
                </form>
            ) : (
